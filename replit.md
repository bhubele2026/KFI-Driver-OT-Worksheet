...
- Optional env: `APP_BASE_URL` (origin used in invite/reset links — falls back to `https://${REPLIT_DOMAINS[0]}`; required in prod if neither is set), `MAIL_FROM` (overrides the mailer's from-address; defaults to `no-reply@kfi-ot.local`), `AI_EXTRACT_PROVIDER` (`claude` | `gemini`, default `claude`), `CLAUDE_EXTRACT_MODEL` (default `claude-sonnet-4-5`), `GEMINI_EXTRACT_MODEL` (default `gemini-2.5-flash`). `SENDGRID_API_KEY` can be set directly as an escape hatch but is normally unnecessary — the mailer pulls credentials from the Replit SendGrid integration via the connector proxy. `AI_INTEGRATIONS_GEMINI_API_KEY` / `AI_INTEGRATIONS_GEMINI_BASE_URL` (auto-provisioned by the Replit Gemini integration) are now only used as a quiet fallback when Claude is unreachable after retries.
